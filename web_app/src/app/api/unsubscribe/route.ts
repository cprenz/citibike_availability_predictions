  ...
  try {
    if (body.email && typeof body.email === "string") {
      ...
      const rows = await querySnowflake(
        "SELECT id, email, station_id, station_name, target_time, created_at FROM subscribers WHERE email = ?",
        [email]
      );
      ...
      for (const row of rows) {
        await executeSnowflake(
          "INSERT INTO unsubscribed (email, station_id, station_name, target_time, subscribed_at) VALUES (?, ?, ?, ?, ?)",
          [row.email, row.station_id, row.station_name ?? null, row.target_time ?? null, row.created_at]
        );
      }
      ...
      const rows = await querySnowflake(
        "SELECT email, station_id, station_name, target_time, created_at FROM subscribers WHERE id = ?",
        [id]
      );
      ...
      const { email, station_id, station_name, target_time, created_at } = rows[0];

      await executeSnowflake(
        "INSERT INTO unsubscribed (email, station_id, station_name, target_time, subscribed_at) VALUES (?, ?, ?, ?, ?)",
        [email, station_id, station_name ?? null, target_time ?? null, created_at]
      );
